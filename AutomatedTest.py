...
# MAGIC %md
# MAGIC # Data Science session: Automated Testing
# MAGIC 
...
try:
  local_path = ModelsArtifactRepository(f"models:/tania-telco-churn/staging").download_artifacts("")
except Exception:
...
latest_model_detail = client.get_latest_versions("tania-telco-churn", stages=['Staging'])[0]
...
output_df = fs.score_batch("models:/tania-telco-churn/staging", spark.table("tania.demographic"))
...
client.transition_model_version_stage("tania-telco-churn", latest_model_detail.version, stage="Production", archive_existing_versions=True)
